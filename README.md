...
| `batch_dirs` | **按一级子目录分批跑**：内存只跟最大子目录有关；整轮一条 R，带批次汇总 | `false` |
| `batch_name_filter` | 分批时只挑名字含该段的子目录 | 空 |
...
### 三种模式彻底隔离
「通用扫描 / 日志筛选回传 / 历史结果重传」三种模式的**配置与结果各自独立**：切模式时左侧栏数据（含扫描目标目录）、表格行、进度节点、目录索引整份切换，互不覆盖；`findany.toml` 里每模式一段（`[modes.<模式>.*]`），顶层段仍是「当前模式」的镜像，`--auto` 与老工具读到的东西不变。

### 大目录：先统计、可续跑、按清单跑
- **目录统计索引**：开跑前先把目录树统计清楚（`_index_<指纹>.jsonl` + `completed` 标记），**统计完成即落盘复用 —— 下次开始一步遍历都不做**；统计被打断也能接着数。
- **文件夹级续跑**：每跑完一个文件夹就记标记，续跑整份跳过；路径比较统一规范化（大小写 / 正反斜杠），不会因为路径写法变了就整批重跑。
- **清单驱动处理**：统计完成后按清单逐目录跑，省掉一整遍全树遍历（新增目录需「从头开始」重新统计）。
- 界面显示「目录（已统计 / 统计中）：N（子目录 M）」与统计出的真实文件总数。

### 历史结果重传（`filter_result.xlsx`）
- 判定口径：**只按「OA3 必需字段是否齐全」**（SN / ProductKeyID / HardwareHash / Baseboard）—— 不看上次回传状态、也不看判型名；字段齐全就重新回传，缺字段跳过。
- **跳过的不写回原文件**；只有真正回传过的行才写回，写回前自动留 `.bak-时间戳` 备份。
- **逐台实时计数**：回传过程中「成功 / 冲突 / 失败」一台一变；文件级状态写明「完成（写回 N 格）」「跳过 N 台（…未写回）」。
- 目标目录就用「扫描目标」里的路径（递归找所有 `filter_result.xlsx`），不再单独弹选目录框。

### 回传链路加固
- CLI 的 **stdout / stderr 各自独立线程读取 + 超时保护**：即使 CLI 是启动器（子进程继承管道导致 EOF 永不出现）也不会把整批卡死。
- CLI 应答（含 `error_code` / `message`）写回「回传错误」列，失败原因一眼可见；读不动 / 写不动的文件记入 `_bad_files.txt`。
- 写回**原子化**（写临时文件 → 校验 → rename 替换）、先探可写、进度节点原子写；内存到上限主动安全停。

### 界面
- 结果表的**「目录」列可点**：点一下直接用资源管理器打开该批次目录。
- 运行中**持续重绘**（修掉「界面空闲 → 后台被有界通道堵住 → 双方互等」的假死）。
- 批次目录名精确到**秒**（同分钟几百批时不再逐个探测）。

## 路线图（v0.3.0）

- **新增日志筛选类型 TP100**（待提供样本后接入提取器与导出模板）。
- **新增「服务器 / 数据库」模式**：从数据库侧导出数据（优先 **e-autotest** 与 **etest** 数据库），**只对接 etest-core**。
